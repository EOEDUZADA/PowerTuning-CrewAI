...
class Crewaisearcher():
    ...
    def html_formatting_task(self) -> Task:
        return Task(
            config=self.tasks_config['html_formatting_task'], # type: ignore[index]
        )
    
    @task
    def file_writing_task(self) -> Task:
        return Task(
            config=self.tasks_config['file_writing_task'], # type: ignore[index]
            output_file=f'report.html'
        )

    @crew
    def crew(self) -> Crew:
        """Creates the Crewaisearcher crew"""
